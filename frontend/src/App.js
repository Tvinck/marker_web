...
function MapPage() {
  ...
  const onSubmitNew = (e) => {
    ...
    addMarker(clientId, { type, title, description, location: selected.coords });
    setSelected(null); setAdding(false); setStore({ ...getStore(clientId) });
      ...
      <Dialog open={showOnboarding} onOpenChange={setShowOnboarding}>
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <Card className="bg-card"><CardHeader className="p-3"><div className="flex items-center gap-2 text-primary"><MapPin size={18}/><span className="font-medium">Метки</span></div></CardHeader><CardContent className="p-3 text-sm text-muted-foreground">Добавляйте точки на карту (тап по карте), описывайте ситуацию.</CardContent></Card>
...
function ProPage() { const clientId = useClientId(); const s = getStore(clientId); const [user, setUser] = useState(s.user); const navigate = useNavigate(); const handleTrial = () => { grantProTrial(clientId); setUser(getStore(clientId).user); toast({ title: "PRO активирован", description: "Пробный месяц подключён (1 ₽) — мок-режим" }); }; const handleBuy = () => { const payment = mockCreateEnotPayment(clientId, { amountRub: 149 }); toast({ title: "Создан платёж (мок)", description: payment.url }); }; const tryPoints = () => { const res = tryActivateProFromPoints(clientId); if (!res.ok) toast({ title: "Недостаточно баллов", description: "Нужно 1000 баллов" }); setUser(getStore(clientId).user); }; return (<MainLayout title="PRO" subtitle="Навигатор, стили карты, камеры, парковки онлайн"><Card><CardHeader><CardTitle>Ваш статус</CardTitle><CardDescription>{user.isPro ? `Активно до ${new Date(user.proUntil).toLocaleDateString()}` : "Обычный аккаунт"}</CardDescription></CardHeader><CardContent className="space-y-3">{!user.isPro && (<div className="flex flex-wrap gap-2"><Button onClick={handleTrial}>Пробный месяц — 1 ₽</Button><Button variant="secondary" onClick={handleBuy}>Купить за 149 ₽/мес</Button><Button variant="outline" onClick={tryPoints}>Активировать за 1000 баллов</Button></div>)}<div className="text-sm text-muted-foreground">PRO даёт: смена стиля карты, бесплатные парковки и онлайн-камеры, камеры ДПС, навигатор и бонусы.</div></CardContent></Card><Card className="mt-3"><CardHeader><CardTitle>Стили карты</CardTitle><CardDescription>Доступно для PRO</CardDescription></CardHeader><CardContent><Tabs defaultValue={getStore(clientId).user.settings.mapStyle}><TabsList>{mapStyles().map((s) => (<TabsTrigger key={s.id} value={s.id}>{s.name}</TabsTrigger>))}</TabsList>{mapStyles().map((s) => (<TabsContent key={s.id} value={s.id}><div className="flex items-center justify-between"><span>Стиль: {s.name}</span><Button size="sm" onClick={() => { if (!getStore(clientId).user.isPro) return toast({ title: "Только для PRO" }); updateUser(clientId, { settings: { ...getStore(clientId).user.settings, mapStyle: s.id } }); toast({ title: "Стиль применён" }); }}>Применить</Button></div></TabsContent>))}</Tabs></CardContent></Card></MainLayout>); }

function ProfilePage() { const clientId = useClientId(); const [user, setUser] = useState(getStore(clientId).user); const { theme, toggle } = useTheme(); const claim = () => { const res = claimDaily(clientId); if (res.ok) toast({ title: "+10 баллов начислено" }); else toast({ title: res.message }); setUser(getStore(clientId).user); }; return (<MainLayout title="Профиль"><Card><CardContent className="p-4"><div className="flex items-center justify-between"><div><div className="text-base font-semibold">{user.name} {user.isPro && <Badge className="ml-2">PRO</Badge>}</div><div className="text-sm text-muted-foreground">Баллы: {user.points}</div></div><div className="flex items-center gap-2"><Button variant="secondary" onClick={claim}>Ежедневная награда</Button><Button variant="outline" onClick={toggle} aria-label="Сменить тему">{theme === "dark" ? <Sun size={16} /> : <Moon size={16} />}</Button></div></div></CardContent></Card></MainLayout>); }

function AdminPage() { const clientId = useClientId(); const [pending, setPending] = useState(getPending(clientId)); if (!isAdmin(clientId)) { return (<MainLayout title="Модерация"><p className="text-sm text-muted-foreground">Доступ запрещён. Вы не администратор.</p></MainLayout>); } const act = (id, ok) => { adminApprove(clientId, id, ok); setPending(getPending(clientId)); toast({ title: ok ? "Метка одобрена" : "Отклонено" }); }; return (<MainLayout title="Модерация">{pending.length === 0 ? (<p className="text-sm text-muted-foreground">Нет новых меток</p>) : (<div className="space-y-2">{pending.map((m) => (<Card key={m.id}><CardHeader><CardTitle className="text-base">{m.title}</CardTitle><CardDescription>{m.description}</CardDescription></CardHeader><CardContent className="flex gap-2"><Button onClick={() => act(m.id, true)}>Одобрить</Button><Button variant="secondary" onClick={() => act(m.id, false)}>Отклонить</Button></CardContent></Card>))}</div>)}</MainLayout>); }

function AdvertisePage() { const clientId = useClientId(); return (<MainLayout title="Реклама" subtitle="Оставьте заявку на сотрудничество"><Card><CardContent className="p-4"><form className="space-y-3" onSubmit={(e) => { e.preventDefault(); toast({ title: "Заявка отправлена (мок)", description: "Мы свяжемся с вами" }); e.currentTarget.reset(); }}><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div><Label>Компания</Label><Input required placeholder="ООО Реклама" /></div><div><Label>Телефон/Telegram</Label><Input required placeholder="@username" /></div></div><div><Label>Комментарий</Label><Textarea placeholder="Опишите задачу" /></div><Button type="submit">Отправить</Button></form></CardContent></Card><div className="mt-4 text-xs text-muted-foreground">Оплата и интеграции будут реализованы через ENOT после подключения бэкенда.</div></MainLayout>); }
